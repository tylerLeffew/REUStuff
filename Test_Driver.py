...
nx,ny = 225,225
big_array = np.zeros((nx,ny),dtype=np.float32)
wx = nx//10 + 1
wy = ny//10 + 1
...
def draw_shadows_on_simple_env():
       ...
       with mapped_png_context("Images/simple_shadow_environment.png",((0,0),(10,10)),(1000,1000)) as context:
              SRSE_Grid = Grid(big_array,.05)
              print(type(SRSE_Grid.occupancy_array))
              print(SRSE_Grid.occupancy_array)
              ...
              add_robot((5,5),context)
              plt.imshow(Shadows.occupancy_array, cmap='gray')
              plt.axis('off')
              plt.show()
...
def moving_shadows_discrete(): #### Work in progress
       ENV_Grid = Grid.Grid(.05,big_array)
       first_path = ((5,4),(1,9))
       second_path = ((1,9),(8,9))
